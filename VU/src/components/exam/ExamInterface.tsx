...
export default function ExamInterface({ examId }: { examId: string }) {
  const CONTINUOUS_CHECK_INTERVAL_MS = 15000;
  const CAMERA_BLOCK_ALERT_MS = 60000;
  const [currentQIndex, setCurrentQIndex] = useState(0);
  const [answers, setAnswers] = useState<Record<string, string>>({});
  const [flagged, setFlagged] = useState<Record<string, boolean>>({});
  const [timeLeft, setTimeLeft] = useState(7200); // 2 hours
  const [submitted, setSubmitted] = useState(false);
  const [cameraNotVisibleMs, setCameraNotVisibleMs] = useState(0);
  const [cameraWarningShown, setCameraWarningShown] = useState(false);
    ...
    <div className="grid grid-cols-1 xl:grid-cols-4 gap-6 h-[calc(100vh-8rem)]">
      ...
      <div className="xl:col-span-1 flex flex-col gap-6">
        ...
        <div className="bg-white rounded-xl shadow-[0_8px_30px_rgb(0,0,0,0.06)] border border-slate-200 p-5">
              ...
              <WebcamMonitor 
                onCapture={async (img) => {
                  try {
                    ...
                    const res = await fetch('http://localhost:8000/api/verify-continuous/', {
                      method: 'POST',
                    ...
                    const noFaceDetected = (data?.error && String(data.error).toLowerCase().includes('no face')) || data?.match_percentage === 0;
                    if (noFaceDetected) {
                      setCameraNotVisibleMs((prev) => {
                        const updated = prev + CONTINUOUS_CHECK_INTERVAL_MS;
                        if (updated >= CAMERA_BLOCK_ALERT_MS && !cameraWarningShown) {
                          setCameraWarningShown(true);
                          setProctoringMessage('Alert: Your face has not been detected for 1 minute. Please reposition now. Continued invisibility may affect your marks.');
                        }
                        return updated;
                      });
                    } else {
                      setCameraNotVisibleMs(0);
                    }

                    if (data && data.verified === false && typeof data.match_percentage === 'number' && data.match_percentage < 40) {
                      setProctoringMessage('Warning: Face match is below 40%. A snapshot has been logged for manual review. Please improve lighting and stay centered.');
                    }
                  } catch (err) {
                    console.error('Continuous verification error:', err);
                    setProctoringMessage('Warning: Proctoring check failed. Your exam continues, but this incident has been flagged for review.');
                  }
                }}
                isVerifying={false}
                status="idle"
                continuousMode={true}
                continuousInterval={CONTINUOUS_CHECK_INTERVAL_MS}
              />
           ...
           {proctoringMessage && (
             <div className="mt-3 rounded-md border border-amber-200 bg-amber-50 p-2 text-[11px] text-amber-800 font-medium">
               {proctoringMessage}
             </div>
           )}
           ...
           <div className="mt-4 text-[11px] text-slate-500 font-medium leading-relaxed bg-slate-50 p-3 rounded-lg border border-slate-100">
             Your webcam feed and screen activity are being securely recorded and analyzed by the DeepFace verification engine. You will receive warnings if your face is not visible, and repeated incidents may lead to mark penalties.
           </div>
        </div>
